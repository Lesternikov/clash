...
class World:
    ...
    def draw_recruitment(self, screen):
        ...
        # ---- LISTA JEDNOSTEK Z PODŚWIETLENIEM W ŚRODKU + wolne miejsca ----
        ...
        scroll_offset = max(0, self.recruitment_scroll - 2)  # 2 wolne miejsca na górze
        visible_units = unit_types[
            scroll_offset:
            scroll_offset + self.visible_recruitment_count
        ]
        highlight_index = self.visible_recruitment_count // 2
        ...
        for i, unit_type in enumerate(visible_units):
            ...
            # kolor podświetlenia w środku
            text_color = (180,180,180)
            if i == highlight_index:
                text_color = (255,255,255)
                self.selected_unit_type = scroll_offset + i

            # wygaszenie zakupionego patentu poza highlight
            if i != highlight_index and any(p is not None and p["unit_type"] == unit_type for p in self.selected_castle.patents):
